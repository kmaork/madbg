...
from .utils import run_in_process, enter_pty, JOIN_TIMEOUT, run_client


def set_trace_script(start_with_ctty, port):
    enter_pty(start_with_ctty)
    madbg.set_trace(port=port)


def set_trace_and_expect_var_to_change_script(start_with_ctty, port) -> bool:
    """
...
def test_set_trace(port, start_debugger_with_ctty):
    debugger_future = run_in_process(set_trace_and_expect_var_to_change_script, start_debugger_with_ctty, port)
...
def test_set_trace_and_quit_debugger(port, start_debugger_with_ctty):
    debugger_future = run_in_process(set_trace_script, start_debugger_with_ctty, port)
